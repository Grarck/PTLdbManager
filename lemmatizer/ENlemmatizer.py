...
bar = tqdm(total=100, mininterval=5)
bar.clear()
...
class ENLemmatizer (object):
    ...
    def lemmatize(self, rawtext, verbose=False, port=None):
        ...
        if rawtext == None or rawtext == '':
            ...
            try:
                if port:
                    response = requests.post(self.__url.replace('7777', str(
                        port)), headers=self.__headers, data=str(data).encode('utf-8'))
                else:
                    response = requests.post(
                        self.__url, headers=self.__headers, data=str(data).encode('utf-8'))
            except:
                print(
                    'Error processing request at the lemmatization service, port:', str(port))
    ...
    def lemmatizeBatch(self, IDTextList, processes=1, verbose=False):
        ...
        """
        bar.reset(total=len(IDTextList)/processes)
